...
void ss::pl::graph::Graph::drawGraph()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        DrawTextEx(font, TextFormat("%.2f", (currentSense)), { 140, static_cast<float>(leftTextY) }, 19, 0, { 108, 108, 108, 255 });
        DrawTextEx(font, TextFormat("%.2f", (currentSpeed)), { static_cast<float>(bottomTextX), 844 }, 19, 0, { 108, 108, 108, 255 });
...
void ss::pl::graph::Graph::checkCollision()
{
    ...
    if (CheckCollisionPointRec(mousePos, { 50, 90, static_cast<float>(backArrow_Texture.width), static_cast<float>(backArrow_Texture.height) }))
    {
