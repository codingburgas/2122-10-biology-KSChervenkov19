...
///
/// Function returning lasted and died entities of each cycle. The died entities are calculated using the previous
/// cycle.
/// @param cycle A cycle of a generation.
/// @return A pair of ints holding lasted and died.
std::pair<int, int> ss::pl::graph::Graph::getLastedAndDied(
    const ss::types::Cycle &cycle) // !WARNING! THE FUNCTION IS UNSAFE!
{
...
void ss::pl::graph::Graph::checkCollision()
{
        ...
        IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
    {
        ...
        if (currentCycle != 1)
        {
            float testing = std::abs(static_cast<float>((cycleInfo[currentCycle - 1].lastedEntities -
                                                         cycleInfo[currentCycle - 2].lastedEntities)) /
                                     cycleInfo[currentCycle - 2].lastedEntities) *
                            100;
            float testing2 = std::abs(static_cast<float>((cycleInfo[currentCycle - 2].lastedEntities -
